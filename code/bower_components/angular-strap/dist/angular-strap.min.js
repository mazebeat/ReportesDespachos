    ...
    angular.module("mgcrea.ngStrap", ["mgcrea.ngStrap.modal", "mgcrea.ngStrap.aside", "mgcrea.ngStrap.alert", "mgcrea.ngStrap.button", "mgcrea.ngStrap.select", "mgcrea.ngStrap.datepicker", "mgcrea.ngStrap.timepicker", "mgcrea.ngStrap.navbar", "mgcrea.ngStrap.tooltip", "mgcrea.ngStrap.popover", "mgcrea.ngStrap.dropdown", "mgcrea.ngStrap.typeahead", "mgcrea.ngStrap.scrollspy", "mgcrea.ngStrap.affix", "mgcrea.ngStrap.tab", "mgcrea.ngStrap.collapse"]), angular.module("mgcrea.ngStrap.affix", ["mgcrea.ngStrap.helpers.dimensions", "mgcrea.ngStrap.helpers.debounce"]).provider("$affix", function () {
        ...
        this.$get = ["$dateFormatter", "$dateParser", "$sce", function (n, a, o) {
            return function (i) {
                var r = i.$scope, s = i.$options, l = s.lang, u = function (e, t) {
                ...
                }), d = n.weekdaysShort(l), f = d.slice(s.startWeek).concat(d.slice(0, s.startWeek)), p = o.trustAsHtml('<th class="dow text-center">' + f.join('</th><th class="dow text-center">') + "</th>"), m = i.$date || (s.startDate ? c.getDateForAttribute("startDate", s.startDate) : new Date), g = {
                    year: m.getFullYear(),
                    month: m.getMonth(),
                    date: m.getDate()
                    ...
                    update: function (e, t) {
                        !this.built || t || e.getFullYear() !== g.year || e.getMonth() !== g.month ? (angular.extend(g, {
                            ...
                            date: i.$date.getDate()
                        }), i.$build()) : e.getDate() !== g.date && (g.date = i.$date.getDate(), i.$updateSelected())
                    },
                    build: function () {
                        ...
                        for (var f, m = [], $ = 0; 42 > $; $++)f = c.daylightSavingAdjust(new Date(o.getFullYear(), o.getMonth(), o.getDate() + $)), m.push({
                            date: f,
        ...
        this.$get = ["$locale", "dateFilter", function (r, s) {
            var l = function (l) {
                ...
                var f, p, m = angular.extend({}, i, l), g = {}, $ = {
                    sss: "[0-9]{3}",
                    ss: "[0-5][0-9]",
                    s: m.strict ? "[1-5]?[0-9]" : "[0-9]|[0-5][0-9]",
                    mm: "[0-5][0-9]",
                    m: m.strict ? "[1-5]?[0-9]" : "[0-9]|[0-5][0-9]",
                    HH: "[01][0-9]|2[0-3]",
                    H: m.strict ? "1?[0-9]|2[0-3]" : "[01]?[0-9]|2[0-3]",
                    hh: "[0][1-9]|[1][012]",
                    h: m.strict ? "[1-9]|1[012]" : "0?[1-9]|1[012]",
                    a: "AM|PM",
                    EEEE: r.DATETIME_FORMATS.DAY.join("|"),
                    EEE: r.DATETIME_FORMATS.SHORTDAY.join("|"),
                    dd: "0[1-9]|[12][0-9]|3[01]",
                    d: m.strict ? "[1-9]|[1-2][0-9]|3[01]" : "0?[1-9]|[1-2][0-9]|3[01]",
                    MMMM: r.DATETIME_FORMATS.MONTH.join("|"),
                    MMM: r.DATETIME_FORMATS.SHORTMONTH.join("|"),
                    MM: "0[1-9]|1[012]",
                    M: m.strict ? "[1-9]|1[012]" : "0?[1-9]|1[012]",
                    yyyy: "[1]{1}[0-9]{3}|[2]{1}[0-9]{3}",
                    yy: "[0-9]{2}",
                    y: m.strict ? "-?(0|[1-9][0-9]{0,3})" : "-?0*[0-9]{1,4}"
                }, h = {
        ...
        this.$get = ["$window", "$document", "$rootScope", "$sce", "$dateFormatter", "$tooltip", "$timeout", function (t, n, a, o, i, r, s) {
            function l(t, n, a) {
                ...
                }, w = i.getDatetimeFormat(p.timeFormat, g), b = i.hoursFormat(w), D = i.timeSeparator(w), k = i.minutesFormat(w), S = i.showAM(w);
        ...
        this.$get = ["$window", "$rootScope", "$compile", "$q", "$templateCache", "$http", "$animate", "$sce", "dimensions", "$$rAF", "$timeout", function (n, a, o, i, r, s, l, u, c, d, f) {
            function p(t, n) {
                ...
                E.$promise.then(function (e) {
                    ...
                    F.delay && angular.isNumber(F.delay) && (F.delay = {
                        show: F.delay,
                        hide: F.delay
                    }), "self" === F.container ? R = t : angular.isElement(F.container) ? R = F.container : F.container && (R = g(F.container)), s(), F.target && (F.target = angular.isElement(F.target) ? F.target : g(F.target)), F.show && V.$$postDigest(function () {
                    ...
                    if (F.bsEnabled) {
                        ...
                        F.container ? (e = R, n = R[0].lastChild ? angular.element(R[0].lastChild) : null) : (e = null, n = t), O && M(), q = E.$scope.$new(), O = E.$element = N(q, function () {
                            ...
                            display: "block",
                            visibility: "hidden"
                        }).addClass(F.placement), F.animation && O.addClass(F.animation), F.type && O.addClass(F.prefixClass + "-" + F.type), F.customClass && O.addClass(F.customClass);
