AmCharts.AmFunnelChart = AmCharts.Class({
    inherits: AmCharts.AmSlicedChart, construct: function (u) {
        this.type = "funnel";
        ...
        if (AmCharts.ifArray(u))if (0 < this.realWidth && 0 < this.realHeight) {
            var t = this.container, D = this.startDuration, k = this.rotate, x = this.updateWidth();
            this.realWidth = x;
            var g = this.updateHeight();
            this.realHeight = g;
            var q = AmCharts.toCoordinate, A = q(this.marginLeft, x), v = q(this.marginRight, x), a = q(this.marginTop, g) + this.getTitleHeight(), q = q(this.marginBottom, g), v = x - A - v, y = AmCharts.toCoordinate(this.baseWidth, v), l = AmCharts.toCoordinate(this.neckWidth, v), z = g - q - a, w = AmCharts.toCoordinate(this.neckHeight,
                z), r = a + z - w;
            k && (a = g - q, r = a - z + w);
            this.firstSliceY = a;
            ...
            for (var h = v / 2 + A, E = (z - w) / ((y - l) / 2), m = y / 2, y = (z - w) * (y + l) / 2 + l * w, B = a, F = 0, w = 0; w < u.length; w++) {
                ...
                if (!0 !== c.hidden) {
                    var n = [], e = [], b;
                    if ("height" == this.valueRepresents)b = z * c.percents / 100; else {
                        var p = -y * c.percents / 100 / 2, C = m;
                        d = -1 / (2 * E);
                        b = Math.pow(C, 2) - 4 * d * p;
                        0 > b && (b = 0);
                        b = (Math.sqrt(b) - C) / (2 * d);
                        if (!k && a >= r || k && a <= r)b = 2 * -p / l; else if (!k && a + b > r || k && a - b < r)d = k ? Math.round(b + (a - b - r)) : Math.round(b - (a + b - r)), b = d / E, b = d + 2 * (-p - (C - b /
                        2) * d) / l
                    }
                    p = m - b / E;
                    C = !1;
        ...
        for (var D = 0, k = this.chartData, x = k.length, g, q = 0; q < x; q++) {
            g = k[x - q - 1];
            var A = g.label, v = g.labelY,
                a = g.labelX, y = g.labelHeight, l = v;
